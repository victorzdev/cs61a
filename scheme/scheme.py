...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(expression)
        new_pair = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(expression, new_pair, env)
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        ...
        if is_false_primitive(re) or expressions.rest == nil:
            return re
...
def do_or_form(expressions, env):
    ...
    while expressions != nil:
        re = scheme_eval(expressions.first, env)
        if is_true_primitive(re) or expressions.rest == nil:
            return re
        expressions = expressions.rest
    return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        values = Pair(scheme_eval(p.rest.first, env), values)
...
# a = scheme_eval(read_line("(let ((x 42)) (list x y))"), create_global_frame())
# b = read_line("(cond ((= '(3 4 5 6) nil) 1)")
# print("a")
@main
def run(*argv):
    import argparse
